/**
 * A single source of truth for icons — Lucide-style 24×24 stroke paths that
 * inherit `currentColor`. Use `icon(name, opts)` anywhere HTML is built; the
 * same set ships as the React `<Icon>` registry component so it's reusable in
 * apps. Adding an icon here adds it to the Foundations gallery automatically.
 */
export const ICONS: Record<string, string> = {
  ...
  "file-lock": '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/><path d="M14 2v5h5"/><rect width="8" height="6" x="8" y="12" rx="1"/><path d="M10 12v-1a2 2 0 1 1 4 0v1"/>',
...
/** Return inline SVG markup for a named icon (falls back to `help`). */
export function icon(name: string, opts: IconOptions = {}): string {
  const paths = ICONS[name] ?? ICONS.help!;
  const size = opts.size ?? 16;
  const cls = opts.cls ? ` class="${opts.cls}"` : "";
  const sw = opts.strokeWidth ?? 2;
  return `<svg${cls} width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${paths}</svg>`;
}

/** All icon names, sorted — used by the Foundations gallery. */

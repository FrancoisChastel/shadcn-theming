...
const chev = '<span class="t-chev">▶</span>';
...
function aiActions(): string {
  return `<div class="ai-actions">
    <button class="ai-act" data-ai-copy title="Copy">⧉</button>
    <button class="ai-act" title="Retry">↻</button>
    <button class="ai-act" title="Edit">✎</button>
    <button class="ai-act" data-ai-vote title="Good response">👍</button>
    <button class="ai-act" data-ai-vote title="Bad response">👎</button>
  </div>`;
}
...
function aiPlan(items: Array<{ state: "done" | "run" | "pending"; text: string }>): string {
  return `<div class="ai-plan"><div class="ai-plan-title">Plan</div>${items
    .map((i) => `<div class="ai-plan-item ${i.state}"><span class="mark">${i.state === "done" ? "✓" : i.state === "run" ? "◐" : ""}</span>${i.text}</div>`)
    .join("")}</div>`;
}

function aiTool(name: string, arg: string, status: "run" | "done", output: string): string {
  const badge = status === "run" ? `<span class="spinner"></span> running` : "✓ done";
...
function aiCode(lang: string, code: string): string {
  return `<div class="ai-code"><div class="ai-code-head"><span>${lang}</span><button class="ai-code-copy" data-ai-copy-code>⧉ Copy</button></div><pre><code>${code}</code></pre></div>`;
}
  ...
  return `<div class="ai-composer">
    <div class="ai-composer-box">
      ...
      <div class="ai-composer-tools">
        <button class="ai-tool-btn" title="Attach file">＋</button>
        <div class="ai-model"><button class="ai-model-trigger" data-menu-trigger>◆ <span data-ai-model-value>Analyst v2</span> ${chev.replace("▶", "▾")}</button><div data-menu><div class="menu-item" data-ai-model-option>Analyst v2</div><div class="menu-item" data-ai-model-option>Analyst Pro</div><div class="menu-item" data-ai-model-option>Fast draft</div></div></div>
        <span class="ai-usage">◷ 3.2k / 200k <span class="bar"><span style="width:2%"></span></span></span>
        <button class="ai-send" data-ai-send title="Send">↑</button>
      </div>
    </div>
  </div>`;
}
...
export function aiSections(brand: string): Section[] {
  const rail = `<div class="ai-rail">
    <div class="ai-rail-head"><button class="btn btn-outline btn-sm" style="width:100%">＋ New chat</button></div>
        ...
        `<div style="width:100%;max-width:640px">
          <div class="ai-empty">
            ...
            <div class="ai-suggest">
              <button data-ai-suggest>📈 Chart world GDP growth with the projection band</button>
              <button data-ai-suggest>🧮 Explain the Phillips-curve regression</button>
              <button data-ai-suggest>📝 Draft a one-paragraph WEO summary</button>
              <button data-ai-suggest>🗺 Compare inflation across regions</button>
            </div>
          </div>
